...
export class EmailApiService {
   ...
   */
  async sendResetPasswordCodeViaMailRu(email: string, code: string): Promise<boolean> {
    try {
      const apiKey = this.configService.get<string>('MAILRU_API_KEY');
      if (!apiKey) {
        console.log('Mail.ru API key not configured, skipping...');
        return false;
      }

      const response = await axios.post('https://send.api.mail.ru/api/v1/email/send', {
        api_key: apiKey,
        from: {
          email: 'noreply@dockmap.com',
          name: 'DockMap'
        },
        to: [{ email }],
      ...
      console.error('Mail.ru API error:', error);
      return false;
    }
  }
   ...
   */
  async sendResetPasswordCodeViaSendGrid(email: string, code: string): Promise<boolean> {
    try {
